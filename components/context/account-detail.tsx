...
function ResourcePill({ label, used, max, icon: Icon }: { label: string; used: number; max: number; icon: React.ElementType }) {
  const pct = max > 0 ? (used / max) * 100 : 0
  const formatBytes = (b: number) => {
  ...
  const fmt = label === "RAM" ? formatBytes : formatUs
  return (
    <div className="flex items-center gap-1.5 bg-muted rounded-md px-2 py-1">
      <Icon className="h-3 w-3 text-muted-foreground shrink-0" />
      <span className="text-xs font-medium">{label}</span>
      <span className="text-[10px] text-muted-foreground">{fmt(used)}/{fmt(max)}</span>
    ...
    <div className="space-y-4">
      ...
      {expanded ? (
        <div className="flex flex-wrap items-center gap-2">
          <ResourcePill label="RAM" used={ram.used || 0} max={ram.quota || 0} icon={HardDrive} />
          <ResourcePill label="CPU" used={cpu.used || 0} max={cpu.max || 0} icon={Cpu} />
          <ResourcePill label="NET" used={net.used || 0} max={net.max || 0} icon={Wifi} />
          <div className="flex items-center gap-1 bg-muted rounded-md px-2 py-1">
            <span className="text-xs text-muted-foreground">CPU Staked</span>
            <span className="text-xs font-medium">{data.cpu_staked || "0"}</span>
          </div>
          <div className="flex items-center gap-1 bg-muted rounded-md px-2 py-1">
            <span className="text-xs text-muted-foreground">NET Staked</span>
            <span className="text-xs font-medium">{data.net_staked || "0"}</span>
          </div>
        </div>
      ) : (
        <>
      ...
      {data.permissions && data.permissions.length > 0 && (
        <>
          ...
          <div className="space-y-2">
            ...
            <div className={cn(expanded && "grid grid-cols-2 gap-2")}>
